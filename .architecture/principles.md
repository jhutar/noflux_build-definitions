...
**Description**: Follow established Bash conventions and leverage the language's specific features and ecosystem effectively.

**Rationale**: Idiomatic Bash is more readable, maintainable, and performant. Fighting the language or importing idioms from other languages leads to friction.

**Application**:
- **Explicit Over Implicit**: Make behavior clear, avoid magic, prefer explicit configuration.
- **Leverage the language tools**: Use built-in features (e.g., `(( a = 40 + 2 ))`).
- **Ecosystem Patterns**: Adhere to shellchecek suggestions.
